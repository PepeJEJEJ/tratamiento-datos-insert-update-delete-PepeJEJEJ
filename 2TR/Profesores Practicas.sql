...
select p.nombre, a.descripcion 
from profesores p 
join imparte i on p.dni = i.dni 
join asignaturas a on a.codigo = i.asignatura;

/*Queremos obtener un listado con:
El nombre del profesor
La asignatura que prepara*/

select p.nombre, a.descripcion
from profesores p
join prepara pr on p.dni = pr.dni
join asignaturas a on pr.asignatura = a.codigo;

/*Haz una consulta que muestre:
El nombre del profesor
La asignatura que imparte
El número de créditos de esa asignatura*/

select p.nombre, a.descripcion, a.creditos
from profesores p
join imparte i on p.dni = i.dni 
...
El nombre del profesor
La asignatura que imparte
El nombre del coordinador de esa asignatura (si existe)*/

select p.nombre, a.descripcion, c.nombre
from profesores p
join imparte i on p.dni = i.dni
join asignaturas a on i.asignatura = a.codigo
left join coordinadores c on a.codigo = c.asig;

/*select profesores.nombre, asignaturas.descripcion, coordinadores.nombre
from profesores
join imparte on profesores.dni = imparte.dni 
join asignaturas on imparte.asignatura = asignaturas.codigo
left join coordinadores on asignaturas.codigo = coordinadores.asig;*/

/*Ejercicio 1 – JOIN múltiple con condición
Muestra el nombre de cada profesor, las asignaturas que imparte y, si las prepara también, indícalo con una columna extra llamada tambien_prepara que diga 'Sí' o 'No'.
Pista: necesitas un LEFT JOIN doble y una expresión condicional (case when).*/

select p.nombre, a.descripcion,
case when pr.asignatura is not null then 'Sí' else 'No' end as tb_prepara
from profesores p
join imparte i on p.dni = i.dni
join asignaturas a on i.asignatura = a.codigo
left join prepara pr on p.dni = pr.dni and pr.asignatura = a.codigo;

/*Enunciado: Muestra todas las asignaturas junto con el coordinador (si existe), los profesores que la imparten y los que la preparan.*/

select distinct a.descripcion, c.nombre, pi.nombre, pp.nombre from asignaturas a
left join coordinadores c on a.codigo = c.asig
left join imparte i on a.codigo = i.asignatura
left join profesores pi on i.dni = pi.dni
left join prepara pr on a.codigo = pr.asignatura
left join profesores pp on pr.dni = pp.dni;

/*Enunciado: Lista todos los profesores que no coordinan, no imparten y no preparan ninguna asignatura.*/
select p.nombre from profesores p
left join imparte i on p.dni = i.dni
left join prepara pr on p.dni = pr.dni
left join coordinadores c on p.dni = c.dni
where i.dni is null and pr.dni is null and c.dni is null;

/*Enunciado: Muestra por cada profesor el número de asignaturas que imparte, prepara y coordina.*/
select p.nombre as profesor,
count(distinct i.asignatura) as num_imparte,
count(distinct pr.asignatura) as num_prepara,
count(distinct c.asig) as num_coordina
from profesores p
left join imparte i on p.dni = i.dni
left join prepara pr on p.dni = pr.dni
left join coordinadores c on p.dni = c.dni
...
/*Enunciado: Muestra parejas de profesores que preparan la misma asignatura (sin repetir parejas).*/
select p1.nombre as profesor1, p2.nombre as profesor2, a.descripcion as asignatura from prepara pr1
join prepara pr2 on pr1.asignatura = pr2.asignatura and pr1.dni < pr2.dni
